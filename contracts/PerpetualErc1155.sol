...
contract PerpetualErc1155 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function deployNewPool(
        ...
        address newPool;
        if (isERC1155(depositToken_)) {
            newPool = address(
                new PoolERC1155(depositToken_, startDate_, maturityDate_, cliff_, msg.sender, rewardTokens_, rewardUnits_)
            );
        } 
        pools.push(newPool);
        emit NewPoolCreated(
